...
void S21Matrix::setRows(int new_rows)
{
  if (new_rows < 1)
  {
    throw std::out_of_range(
        "Incorrect input, rows should not be less than 1");
  }
  if (new_rows != rows_)
  {
    if (rows_ == 0 || cols_ == 0)
...
void S21Matrix::setCols(int new_cols)
{
  ...
  {
    throw std::out_of_range(
        "Incorrect input, cols should not be less than 1");
  }
  if (new_cols != cols_)
  {
    if (rows_ == 0 || cols_ == 0)
...
void S21Matrix::FillMinorMatrix(int current_row, int current_col, S21Matrix &minor_matrix) const
{
  int minor_rows = 0;
